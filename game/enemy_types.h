 ...
 *
 * Map markers (see map.c) are one ASCII letter per species. Lowercase 'k' is
 * reserved for keys — thug uses 'T' (not 'K'). To add a new foe: bump
...
#define enemy_glyph_is_foe(g) \
    ((g) == G_FOE_GOBLIN || (g) == G_FOE_RAT || (g) == G_FOE_THUG)
