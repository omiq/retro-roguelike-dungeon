 ...
 * Sizing (map_w / map_h)
 * -----------------------
 * Templates are authored at ROOM_W x ROOM_H but the visible map is
...
/* Static room table: one string per row, null-padded by C to next string.
 * When map_w is less than ROOM_W, only the left prefix of each row is read.
 * Rows past map_h are ignored after clipping. */
